...
var remoteUrl =
    "https://api.eallion.com/memos/api/v1/memo?creatorId=" +
    memo.creatorId +
    "&rowStatus=NORMAL";
//var remoteUrl = memosHost + "api/v1/memo?creatorId=" + memo.creatorId + "&rowStatus=NORMAL"
var page = 1,
    offset = 10,
    nextLength = 0,
    nextDom = "";
...
function getFirstList() {
    ...
    //var memoUrl_first = "https://api.eallion.com/memos/api/v1/memo?creatorId=101&rowStatus=NORMAL&limit=1&offset=0";
    fetch(memoUrl_first)
        .then((res) => res.json())
        .then((resdata) => {
            updateHTMl(resdata);
        ...
        if (data[i].relationList && data[i].relationList.length > 0) {
            ...
            await Promise.all(
                relationList.map(async (relation) => {
                    ...
                    if (relatedMemoId) {
                        var res = await fetch(
                            "https://api.eallion.com/memos/api/v1/memo/" + relatedMemoId
                        );
                        var resdata = await res.json();

                        const IMG_REG = /\!\[(.*?)\]\((.*?)\)/g;
                        const LINK_REG = /\[(.*?)\]\((.*?)\)/g;
                        const relatedContent = escapeHtml(resdata.content)
                            .replace(IMG_REG, "")
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    var galleryUrl =
        memoUrl + "api/v1/memo/all?rowStatus=NORMAL&tag=相册&limit=" + albumLimit;
    let nowNum = 0;
    fetch(galleryUrl)
        .then((res) => res.json())
        .then((resdata) => {
