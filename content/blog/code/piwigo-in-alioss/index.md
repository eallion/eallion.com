...
summary: 通过源码编译将阿里云 OSS 挂载为 ECS 数据盘，支持 PHP 解析并利用其低成本优势搭建私人网盘。详细步骤包括安装依赖、编译 ossfs、配置
  Bucket 权限及挂载目录，同时提供卸载方法。随后基于挂载的 OSS 部署 piwigo 相册，涵盖下载解压、数据库配置及安装流程，最终实现可自定义优化的相册系统！
tags:
- 阿里云
- oss
- piwigo
