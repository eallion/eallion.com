...
images: ["https://api.eallion.com/og?title=NeoDB%20API%20%E5%88%9B%E5%BB%BA%E8%A7%82%E5%BD%B1%E9%A1%B5%E9%9D%A2"]
authors: ["eallion"]
categories: ["代码"]
tags: 
  - hugo
                    ...
                    <div class="cover">
                        <div class="cover__container">
                            {{ range .item.external_resources }}
                                {{ if (in .url "douban") }}
                                    <a href="{{ .url }}" target="_blank" rel="noreferrer noopener nofollow"><img alt="{{ $title }}" class="lazy" loading="lazy" data-src="https://images.eallion.com/images/neodb/{{ $cover_name }}"></a>
                                {{ end }}
                            {{ end }}
                        </div>
                    </div>
