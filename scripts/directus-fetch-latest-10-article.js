 ...
 * @returns {Array} 文章数据的数组
 */
async function fetchAllArticles() {
  console.log('正在获取最新 10 篇非草稿文章...');
  const articles = [];
  const limit = 10;

  try {
    // 确保只获取非草稿文章
    const url = `${DIRECTUS_API_URL}items/Article?fields=*,tags.Tag_id.*,categories.Category_id.*,authors.Author_id.*,serieses.Series_id.*,featureimage.*&sort=-date&filter[draft][_eq]=false&limit=${limit}`;
