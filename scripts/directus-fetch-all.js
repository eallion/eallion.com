...
// Mastodon API 配置
const MASTODON_API_URL = 'https://e5n.cc/api/v1/accounts/111136231674527355/statuses';
...
/**
 * 从 Mastodon API 获取数据（专门用于相册）
 * @returns {Promise<Array>} 返回 Mastodon 状态数据数组
 */
async function fetchAllAlbumData() {
  console.log('正在获取 Mastodon 相册数据...');
  ...
  try {
    while (true) {
      // 构造请求 URL
      let requestUrl = MASTODON_API_URL + '?only_media=true&limit=40';
      if (maxId) {
    ...
    console.log(`成功获取 ${allData.length} 条符合条件的 Mastodon 相册数据`);
    return allData;
  } catch (error) {
    console.error('获取 Mastodon 相册数据时发生错误：', error);
    return [];
  }
}
...
async function fetchMastodonData() {
  ...
  try {
    const response = await fetch(MASTODON_API_URL + '?limit=30&exclude_replies=true&exclude_reblogs=false');
...
function saveAlbumDataToJson(albumData) {
  ...
  const processedData = albumData.map(status => {
  ...
  // 写入 JSON 文件
  fs.writeFileSync(ALBUM_JSON_PATH, JSON.stringify(processedData, null, 2), 'utf-8');
  console.log(`成功保存 ${processedData.length} 条 Mastodon 相册数据到 ${ALBUM_JSON_PATH}`);
}
...
/**
 * 保存 Mastodon 数据到 JSON 文件（用于 Mastodon 页面）
