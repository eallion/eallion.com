...
async function fetchLatest10Articles() {
  ...
  const response = await fetch(`${DIRECTUS_API_URL}items/Article?fields=*,tags.Tag_id.*,categories.Category_id.*,authors.Author_id.*,serieses.Series_id.*,featureimage.*&sort=-date&filter[draft][_eq]=false`, {
    headers: {
